...
async function showDetail(stock) {
  ...
  document.getElementById('modalContent').innerHTML = `
    ...
    <div id="optionsDetailSection"></div>
  ...
  // ── Load timeline chart + options detail ────────────────────────────────────
  loadTimeline(stock.ticker);
  renderOptionsDetail(stock);
...
function renderOptionsDetail(stock) {
  const el = document.getElementById('optionsDetailSection');
  if (!el) return;
  if (isJapanIndex()) return;
  const opt = (stock.us_advanced || {}).options_detail;
  if (!opt || opt.gex_score == null) return;
  ...
  el.innerHTML =
    ...
    '<div class="mb-6 bg-white dark:bg-gray-900 rounded-xl border border-slate-200 dark:border-gray-800 p-4">' +
      '<div class="flex items-center justify-between mb-3">' +
        ...
        '</div>' +
      '</div>' +
      '<div class="grid grid-cols-3 gap-2 mb-3">' + cells + '</div>' +
